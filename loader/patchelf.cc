...
{
    struct stat st;
//     std::cerr << "stat(fileName.c_str(), &st returns " <<  stat(fileName.c_str(), &st);
...
Elf_Shdr & ElfFile<ElfFileParamNames>::findSection(const SectionName & sectionName)
{
    ...
    if (!shdr) {
        ...
        std::cerr << "cannot find section " << sectionName << "\n";
//         return 0;
    }
    return *shdr;
}
...
string ElfFile<ElfFileParamNames>::getInterpreter()
{
    ...
    if (&shdr == 0) return string("No Interp Found");
...
void ElfFile<ElfFileParamNames>::printNeededLibs(const char * lib, const char * parent, int depth, int mode)
{
    ...
    for (; rdi(dyn->d_tag) != DT_NULL; dyn++) {
        if (rdi(dyn->d_tag) == DT_NEEDED) {
            ...
            auto namefullb_ = std::string(path2) + name;
            auto namefullc_ = std::string(path3) + name;
            const char * namefull = namefull_.c_str();
            const char * namefullb = namefullb_.c_str();
            const char * namerel = namefullc_.c_str();
            if(access(namefull, F_OK) == 0)
            {
                errno = 0;
                if (mode == LDD) {
    if (q == 0)
...
extern "C"
#endif
int print_interp(const char * lib) {
    fileName=lib;
...
extern "C"
#endif
int print_soname(const char * lib) {
    fileName=lib;
    printInterpreter = false;
    printNeeded = false;
    printSoname = true;
...
#ifdef M
extern "C" int bytecmpcqr(void const * p, void const * pp);
